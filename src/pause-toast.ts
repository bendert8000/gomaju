...
const pause = readInjected<{ minutes: number }>("__GOMAJU_PAUSE__", { minutes: 10 });
...
stayEl.addEventListener("click", () => {
  ...
  invoke("cmd_stay_paused_from_reminder").catch((err) =>
    console.error("gomaju: stay-paused reminder action failed", err),
  );
});
...
resumeEl.addEventListener("click", () => {
  ...
  invoke("cmd_resume_from_pause_reminder").catch((err) =>
    console.error("gomaju: resume reminder action failed", err),
  );
});
